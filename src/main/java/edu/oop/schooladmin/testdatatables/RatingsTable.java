...
public class RatingsTable extends QueryableBase<Rating> {
	...
	public static final RatingsTable instance() {
		if (instance == null) {
	...
	@Override
	protected Integer getEntryId(Rating entry) {
		return entry.getRatingId();
	}

	@Override
	protected void setEntryId(Rating entry, int id) {
		entry.setRatingId(id);
	}

	@Override
	protected Rating getCopyOf(Rating entry) {
		return new Rating(entry);
	}

	private static final List<Rating> testData = Arrays.asList(
